...
content = open('LINE_TAI.txt', 'rb').read()
...
with open('counts.json', 'w', encoding='utf-8') as json_file:
    json.dump(counts, json_file, ensure_ascii=False, indent=2)
...
while len(top_words) <= 10:
    i += 1
    word, count = items[i]
    if word == "通話" or word == "照片" or word == "影片" or word == "貼圖" or word == "Cassie" or word == "TAI":
        continue
...
chart2.add_series(
    [counts.get("照片", 0), counts.get("影片", 0), counts.get("通話", 0)])

chart3 = Pie("傳送訊息量")
chart3.set_options(labels=['Cassie', 'TAI'], inner_radius=0)
chart3.add_series([counts.get("Cassie", 0), counts.get("TAI", 0)])

chart.render(dest="關鍵字.html")
chart2.render(dest="通話影片照片數統計.html")
